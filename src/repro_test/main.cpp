// GCC args reproducing the warning: -std=c++23 -Wdangling-pointer -O3
// usually also called with -Werror
// MSVC equivalent args that are fine: /O2 /std:c++20
...
using stop_list = intrusive_list<stop_list_node, &stop_list_node::next, &stop_list_node::prev>;
...
#define TEST(func) void func(); test_base func ## _instance{func, #func, __LINE__}; void func()

test_base * head_test = nullptr;
test_base * crt_test = nullptr;
...
void run(CoTask co_task)
{
  ...
  context ctx{ main_stop_source.get_token() };
