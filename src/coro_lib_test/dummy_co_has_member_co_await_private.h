...
{
  template<typename T>
  class [[nodiscard]] dummy_co_has_member_co_await_private
  {
  public:
    class promise_type : public coro::promise_base<T>
    {
      friend dummy_co_has_member_co_await_private;
    ...
    public:
      ...
      dummy_co_has_member_co_await_private get_return_object() noexcept
      {
  ...
  private:
    ...
    dummy_co_has_member_co_await_private(std::coroutine_handle<promise_type> child_coro) noexcept :
      unique_child_coro_{ child_coro }
    {
    }

  public:
    dummy_co_has_member_co_await_private(const dummy_co_has_member_co_await_private&) = delete;
    dummy_co_has_member_co_await_private& operator=(const dummy_co_has_member_co_await_private&) = delete;
